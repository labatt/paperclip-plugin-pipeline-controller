...
async function parsePipelineDoc(ctx, issueId, companyId) {
    try {
...
async function handleIssueCompleted(ctx, event) {
    ...
    const completedStepIndex = pipeline.steps.findIndex((step) => assignee === step.agentId || assignee.startsWith(step.agentId.slice(0, 8)));
    ...
    if (nextStepIndex < pipeline.steps.length) {
        ...
        const existing = (await ctx.state.get(progressKey));
        const progress = {
        ...
        // Pipeline complete
        await ctx.issues.createComment(parentId, `Pipeline complete! All ${pipeline.steps.length} steps finished. Last step: ${completedStep.agent} (${completedStep.role}).`, companyId);
        // Mark parent as done
...
// ─── Stuck Detection Job ────────────────────────────────────────────────────
async function runStuckDetection(ctx, job) {
    const config = await getConfig(ctx);
    ...
    for (const company of companies) {
        ...
        if (stuckIssues.length > 0) {
            ...
            if (filteredStuck.length > 0) {
                const issueIdentifier = (i) => i.identifier ?? i.id.slice(0, 8);
                const lines = filteredStuck.map((entry) => `- <b>${issueIdentifier(entry.issue)}</b>: ${entry.issue.title} -- ${entry.reason}`);
...
async function registerDataHandlers(ctx) {
    ctx.data.register("pipeline-status", async (params) => {
        ...
        if (!companyId)
            return { activePipelines: [], stuckIssues: [], recentCompletions: [] };
        ...
        for (const issue of allIssues) {
            ...
            if (!issue.parentId) {
                ...
                if (pipeline && pipeline.steps.length > 0) {
                    ...
                    const progress = (await ctx.state.get(progressKey));
