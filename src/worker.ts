...
function eventColor(event: NotificationPayload["event"]): number {
  switch (event) {
...
function eventColorSlack(event: NotificationPayload["event"]): string {
  switch (event) {
 ...
 */
function resolveChannel(config: PipelineControllerConfig): NotificationChannel | null {
...
async function sendViaEmail(
  ...
  if (!channel.emailEndpoint) return;
  const headers: Record<string, string> = {
    "Content-Type": "application/json",
    ...(channel.webhookHeaders ?? {}),
  };
...
async function handlePipelineSavedOnActiveIssue(
  ctx: PluginContext,
  issueId: string,
  companyId: string,
  data: PipelineData,
  ...
  const matchIdx = data.steps.findIndex(
    (step) => assignee === step.agentId || assignee.startsWith(step.agentId.slice(0, 8)),
  );
  ...
  if (matchIdx === -1) {
    ...
    if (data.steps.length > 0) {
      ...
      await ctx.issues.createComment(
        issueId,
        `Pipeline attached: no current assignee matches any step. Reassigning to ${firstStep.agent} (step 1/${data.steps.length}).`,
        companyId,
      );
      const history = [{
        stepIndex: 0,
  ...
  if (status === "done") {
    ...
    const history: PipelineData["stepHistory"] = data.steps.slice(0, matchIdx + 1).map((step, i) => ({
      stepIndex: i,
      agent: step.agent,
      status: "done" as const,
      completedAt: new Date().toISOString(),
    ...
    if (nextIdx < data.steps.length) {
      ...
      await ctx.issues.createComment(
        issueId,
        `Pipeline attached to already-completed issue. Steps 1-${matchIdx + 1} marked done. Advancing to step ${nextIdx + 1} (${nextStep.agent} - ${nextStep.role}).`,
        companyId,
      );
      ...
      await ctx.issues.createComment(
        issueId,
        `Pipeline attached: issue already completed at final step (${data.steps[matchIdx]!.agent}). Pipeline marked complete.`,
        companyId,
      );
      await setPipelineData(ctx, issueId, {
        ...data,
    ...
    const history: PipelineData["stepHistory"] = data.steps.slice(0, matchIdx).map((step, i) => ({
      stepIndex: i,
      agent: step.agent,
      status: "done" as const,
      completedAt: new Date().toISOString(),
    ...
    if (matchIdx > 0) {
      await ctx.issues.createComment(
        issueId,
        `Pipeline attached to in-progress issue. Steps 1-${matchIdx} marked as complete. Current step: ${matchIdx + 1} (${data.steps[matchIdx]!.agent} - ${data.steps[matchIdx]!.role}).`,
        companyId,
      );
    }
...
function getStepErrorPolicy(step: PipelineStep, config: PipelineControllerConfig): { policy: ErrorPolicy; maxRetries: number } {
...
async function getRetryCount(ctx: PluginContext, issueId: string, stepIndex: number): Promise<number> {
...
async function incrementRetryCount(ctx: PluginContext, issueId: string, stepIndex: number): Promise<number> {
  const key = { scopeKind: "issue" as const, scopeId: issueId, stateKey: STATE_KEYS.retryCounter };
  const counters = ((await ctx.state.get(key)) as Record<string, number> | null) ?? {};
...
async function handleRunFailed(ctx: PluginContext, event: PluginEvent): Promise<void> {
  const payload = event.payload as Record<string, unknown> | undefined;
  if (!payload) return;

  const issueId = (payload.issueId as string) ?? event.entityId;
  if (!issueId) return;
  ...
  const failureReason = (payload.error as string) ?? (payload.reason as string) ?? "Unknown error";
  ...
  const assignee = issue.assigneeAgentId ?? "";
  const stepIndex = pipelineData.steps.findIndex(
    (step) => assignee === step.agentId || assignee.startsWith(step.agentId.slice(0, 8)),
  );
  ...
  if (policy === "retry") {
    ...
    if (retryCount < maxRetries) {
      ...
      await ctx.issues.createComment(
        parentId,
        `Step ${stepIndex + 1} (${step.agent}) failed: ${failureReason}. Retrying (${newRetryCount}/${maxRetries}).`,
        companyId,
      );
    ...
    await ctx.issues.createComment(
      parentId,
      `Step ${stepIndex + 1} (${step.agent}) failed: ${failureReason}. Skipping per error policy.`,
      companyId,
    );
...
async function handleEscalation(
  ...
  history: PipelineData["stepHistory"],
  historyEntry: PipelineData["stepHistory"][0] | undefined,
  failedIssue: Issue,
): Promise<void> {
    ...
    for (const issue of allIssues) {
      ...
      if (!issue.parentId) {
        ...
        if (data && data.steps.length > 0 && issue.status !== "done") {
          const ident = (issue as unknown as Record<string, string>).identifier ?? issue.id.slice(0, 8);
...
async function registerActionHandlers(ctx: PluginContext): Promise<void> {
  ...
  ctx.actions.register("get-pipeline", async (params) => {
    ...
    const data = await getPipelineData(ctx, issueId);
    if (!data) return { exists: false, pipeline: null };
  ...
  ctx.actions.register("advance-pipeline", async (params) => {
    ...
    if (nextIdx >= data.steps.length) {
      ...
      await ctx.issues.createComment(
        issueId,
        `Pipeline manually advanced. All ${data.steps.length} steps complete.`,
        companyId,
      );
    ...
    await ctx.issues.createComment(
      issueId,
      `Pipeline manually advanced from step ${currentIdx + 1} (${currentStep.agent}) to step ${nextIdx + 1} (${nextStep.agent}).`,
      companyId,
    );
  ...
  ctx.actions.register("retry-step", async (params) => {
    ...
    if (stepIndex < 0 || stepIndex >= data.steps.length) throw new Error("Invalid step index");
    ...
    const failedEntry = history.find(
      (h) => h.stepIndex === stepIndex && (h.status === "failed" || h.status === "skipped"),
    );
    ...
    await ctx.issues.createComment(
      issueId,
      `Step ${stepIndex + 1} (${step.agent}) manually retried (attempt ${retryCount}).`,
      companyId,
    );
